...
int ballProcessDelay = 0;					 //milliseconds between processing of ball - controls ball speed (set from mode params)
...
const int tiltDelayMillis = 2;					//milliseconds delay between processing tilt reading (again, to de-tune the tilt response a bit)
const int joystickDelayMillis = 7;				//milliseconds delay between processing joystick readings (to also de-tune the joystick response some)
...
//the setup method of the program - happens just once, when power is first applied (or after reset)
void setup(){
//	Serial.begin(115200);
...
//the main loop of the program - happens continuously, as long as the device is powered
void loop(){
	readPaddle();
...
void processBall(void){
	...
	static int lastBallY = 0;						//prior Y position of ball - used for erasing ball when it moves to a new position
...
//Serial.print("ballOnPaddleX:paddleSections:paddleSection:ballXComp:ballYComp: ");
//Serial.print(ballOnPaddleX);
////Serial.print((paddleX + paddleW/2 - ballW/2) - ballX);
//Serial.print(":");
//Serial.print(modeParams[paddleMode].paddleSections);
////Serial.print(((paddleW - ballW)/2 + ballW - 1)/5);
//Serial.print(":");
//Serial.print(paddleSection);
////Serial.print(((paddleX + paddleW/2 - ballW/2) - ballX) / 5;//((paddleW - ballW)/2 + ballW - 1)/3);
//Serial.print(":");
//Serial.print(ballXComp);
//Serial.print(":");
//Serial.println(ballYComp);
...
void getMode() {
	...
	//wait for a mode button to be presseed
...
//		Serial.println();
	...
	const unsigned char brickColors[8][3] = {
//			{0, 0, 255},		//red
//			{0, 100, 255},		//orange
//			{225, 0, 255},		//pink
//			{255, 51, 153},		//purple
